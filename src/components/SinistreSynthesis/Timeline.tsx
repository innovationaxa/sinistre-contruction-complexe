          ...
          {timeline.map((event, index) => {
            ...
            return (
              <div key={index} className="relative flex items-center">
                ...
                <div 
                  className={`absolute flex items-center justify-center w-6 h-6 rounded-full ${getStatusColor(event.statut)} shadow-sm hover:scale-105 transition-transform duration-200`} 
                  style={{ left: '-3.25rem' }}
                >
                ...
                <div className="w-full">
                  <div className={`bg-white rounded-md border shadow-sm hover:shadow-md transition-all duration-300 p-3 ${
                    event.statut === 'completed' ? 'border-green-200' :
                    event.statut === 'upcoming' ? 'border-blue-200' :
                    'border-gray-200'
                  }`}>
                    ...
                    <div className="space-y-1 text-xs">
                      ...
                      {event.details.montant && (
                        <div className="flex items-center gap-1.5">
                          <Euro className="h-2.5 w-2.5 text-green-600" />
                          <span className="text-gray-700">
                      ...
                      <div className="mt-2 pt-2 border-t border-gray-100">
                        <div className="flex flex-wrap gap-1">
                          {event.details.documents.map((doc, docIndex) => (
                            <span key={docIndex} className="inline-block px-1.5 py-0.5 bg-gray-100 text-gray-600 rounded text-xs">
                              {doc}
                            </span>
                          ))}
                        </div>
